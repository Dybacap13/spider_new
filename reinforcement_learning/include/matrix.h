...
unsigned int Matrix::get_row() {  // показать строку
    return (rows);
}


unsigned int Matrix::get_col() {  // показать столбец
    return (cols);
}

double* Matrix::get_data(){  // показыть линию
    return (data);
}
...
void Matrix::eye_zero(){
    for (unsigned int idx = 0; idx < rows; idx++){
        data[idx + cols * idx] = 0.0;
    }
}
...
void Matrix::fill_random(const unsigned int max_range) {
    for (unsigned int idx = 0; idx < rows * cols; idx++){
        data[idx] = (double)(rand())/RAND_MAX * max_range;
    }
}
